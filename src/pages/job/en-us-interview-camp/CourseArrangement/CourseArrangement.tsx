...
const MobileView = ({ data, className }: Props) => {
  ...
  return (
    <div
      className={clsx(
        "bg-white rounded-md flex flex-row",
        styles.m_course_arrangement
      )}
    >
      <div className={clsx("flex-1 rounded-md relative", styles.m_top)}>
        <Image
          src={mCourseArraBannerImage}
          alt="banner"
          width={400}
          height={300}
          sizes="100vw"
        ...
        <ul>
          {data?.map((item, index) => {
            return (
              <li
                key={item.id}
                className={clsx("flex flex-row cursor-pointer", styles.m_box, {
                  [styles.m_active]: activeIndex === index,
                })}
                onMouseEnter={() => setActiveIndex(index)}
              >
                <div className={clsx("", styles.m_day_title)}>{`Day ${
                  index + 1
                } Live`}</div>
                <div className="flex flex-row items-start flex-1">
                  <Image
                    src={circlePlayIconImg}
                    alt="play"
                    className={styles.m_play_icon}
                  />
                  <div className={clsx("")}>
                    <h1 className={clsx("", styles.m_course_title)}>
                      {item.title}
                    </h1>
                    <h2 className={clsx("", styles.m_course_subtitle)}>
...
const PCView = ({ data, className }: Props) => {
  ...
  return (
    <div
      className={clsx(
        "bg-white p-8 rounded-md flex flex-row",
        styles.course_arrangement
      )}
    >
      <div className={clsx("flex-1 rounded-md relative", styles.left)}>
        <Image
          src={courseArraBannerImage}
          alt="banner"
          width={400}
          height={300}
          style={{
        ...
        <ul>
          {data?.map((item, index) => {
            return (
              <li
                key={item.id}
                className={clsx("flex flex-row cursor-pointer", styles.box, {
                  [styles.active]: activeIndex === index,
                })}
                onMouseEnter={() => setActiveIndex(index)}
              >
                <div className={clsx("", styles.day_title)}>{`Day ${
                  index + 1
                } Live`}</div>
                <div className="flex flex-row gap-2 items-start flex-1">
                  ...
                  />
                  <div className={clsx("")}>
                    <h1 className={clsx("", styles.course_title)}>
                      {item.title}
                    </h1>
                    <h2 className={clsx("", styles.course_subtitle)}>
                      {item.content?.map((contentItem) => {
                        return <div key={contentItem}>{contentItem}</div>;
                      })}
                    </h2>
